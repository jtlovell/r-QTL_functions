plot.qtlci<-function(  cross,
  ...
  ggplot(marker.info)+
    geom_segment(aes(x=pos, xend = pos, y=seg.start, yend =seg.end))+
    geom_segment(aes(x=chr.beg, xend = chr.len, y=chr.ys, yend =chr.ys),alpha=.1, data=chr.info)+
    geom_point(aes(x=pos.center,y=numnames, col=cat), data=toplot)+
    geom_segment(aes(x=pos.left, xend=pos.right, yend=numnames,y=numnames,col=cat), data=toplot)+
